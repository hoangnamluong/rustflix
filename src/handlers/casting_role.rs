...
use crate::{
    ...
    models::casting_role::{self, CastingRoleCreateDTO},
...
async fn get_all(pool: web::Data<AppState>) -> impl Responder {
    ...
    let result = web::block(move || casting_role::CastingRole::get_all(&mut conn))
        .await
...
async fn create(pool: web::Data<AppState>, role: web::Json<CastingRoleCreateDTO>) -> impl Responder {
    ...
    let result = web::block(move || {
        casting_role::CastingRole::create(&mut conn, &role)
    })
    .await
    .map_err(|err| format!("Error executing database query: {}", err))
    .and_then(|r| r.map_err(|e| e.to_string()));
